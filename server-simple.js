...
app.post('/api/ia-cartorio', verificarToken, iaCartorioLimiter, async (req, res) => {
  ...
  try {
    ...
    const minutaComFatosFornecidos = ['email', 'nota'].includes(modo) && !usarPesquisaWeb;
    const classificacao = minutaComFatosFornecidos
      ? 'ROTINA'
      : (fontesUtilizadas.length ? (respostaEstruturada.classificacao === 'ROTINA' ? 'ROTINA' : 'ATENCAO') : 'OFICIAL');
    const alertas = [...new Set([...(respostaEstruturada.alertas || []), 'Em teste: confira informações relevantes antes de utilizá-las.', ...(fontesUtilizadas.length || minutaComFatosFornecidos ? [] : ['Nenhuma evidência normativa específica foi localizada nesta pesquisa; a resposta não substitui fundamentação jurídica.'])])].slice(0, 5);
    ...
    const resposta = { level: classificacao, classificacao, title: modo === 'email' ? 'Minuta para revisão' : modo === 'nota' ? 'Minuta de nota para revisão' : 'Orientação da IA Cartório Dias de Castro — teste', text: respostaEstruturada.resposta, resposta: respostaEstruturada.resposta, basis: fontesUtilizadas.length ? `Fontes consultadas: ${fontesUtilizadas.map((fonte) => fonte.documento).join(' e ')}.` : (minutaComFatosFornecidos ? 'Minuta preparada com as informações fornecidas nesta conversa; confirme os dados do pedido antes do envio.' : 'Resposta operacional em teste, sem evidência normativa específica localizada.'), nextStep: minutaComFatosFornecidos ? proximoPassoMinuta : (respostaEstruturada.motivo_escalonamento || `Revise a orientação e encaminhe ao Oficial se houver situação excepcional ou risco registral. Restam ${Math.max(0, IA_CARTORIO_DAILY_LIMIT - usadasHoje - 1)} consultas de IA hoje.`), provider, consultasRestantes: Math.max(0, IA_CARTORIO_DAILY_LIMIT - usadasHoje - 1), fundamentos: fontesUtilizadas, orientacao_interna: respostaEstruturada.orientacao_interna || null, alertas, motivo_escalonamento: minutaComFatosFornecidos ? null : (respostaEstruturada.motivo_escalonamento || null), pesquisa_web: usarPesquisaWeb };
